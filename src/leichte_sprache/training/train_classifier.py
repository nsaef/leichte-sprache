...
def main():
    ...
    if not isinstance(training_args.learning_rate, float):
        training_args.learning_rate = float(training_args.learning_rate)

    os.environ["MLFLOW_EXPERIMENT_NAME"] = (
        os.getenv("MLFLOW_EXPERIMENT_NAME") + "_classifier"
    )
    ...
    if data_args.metric_name is not None:
        ...
        if is_regression:
            ...
            if is_multi_label:
                ...
                logger.info(
                    "Using accuracy, f1, precision and recall as classification scores, you can use --metric_name to overwrite."
                )
